...
export const ConverterMainForm: FC = () => {
  ...
  useEffect(() => {
    ...
    //установливая в качестве валюты по умолчанию самую первую из списка доступных валют
    if (currencyDataList) {
  ...
  if (isLoading) {
    return <div>Data downloading by currencies is in progress...</div>;
  }

  if (requestError) {
    return (
      <div>Failed to load currency data... {requestError.errorMsg}</div>
    );
  }
